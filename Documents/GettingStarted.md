...
大会へ参加するには、connpassの募集ページからエントリーし、Connpass受付番号を取得してください。
...
| 項目 | 内容 |
| --- | --- |
| 制作者名 | 配信や結果表示などで使用する作成者名です。 |
| 所属（学校・団体名） | 制作者の所属です。任意で入力できます。 |
| パーティー名 | 作成するパーティーの名前です。 |
| パーティーの紹介 | パーティーの戦略や特徴を紹介する短い説明です。 |
| パーティーアイコン | 配信や観戦画面などで使用するアイコンです。 |


パーティーアイコンには、`Assets/Dungeon/GameAssets/Icons/` に用意されたサンプル画像を自由に割り当てられます。

自分で用意した画像を使用する場合は、その画像を自分の参加者用フォルダ内に配置してから、パーティーアイコンに設定してください。フォルダ外の自作画像を参照したまま提出はできません。

設定を変更したら、Prefab Mode上部の **Save** を押して保存してください。
...
試合中にキーボードの **Shift + ↑** を押すと、実行速度を速められます。AIの動作確認や、試合結果を繰り返し確認するときに利用してください。
